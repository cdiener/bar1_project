...
void alphaAbsorption(const GV& gv, const GFS& gfs,const PGMap& pg, T &absorption, const X& x, const double dt)
{
    ...
    for (IntersectionIterator is = gv.ibegin(*eit); is!= gv.iend(*eit); ++is)
    {
      if(is->boundary())
      {
        ...
        if(physgroup_index > N_ALPHA) {
         ...
         temp[physgroup_index - N_ALPHA -1] += alpha*length; //*SIGMA*dt;
        }
      }
    }
  }
...
/**
 *
 * \brief 	this function integrates the alpha-factor absorption of a given MATa-Cell
 *
 * \param  	dt    		length of time step
 * \param  	t_start    	start point for integration
 * \param  	t_end    	end point for integration
 * \param	cellNumber	identifier for the corresponding MATa-cell
 * 
 * \return	value of the integral
 */

// integrates alpha concentration over the cell surface at a given timepoint

template<typename RF>
RF integrateAlpha(RF dt, RF timepoint, int cellNumber)
{
  int index = (int)(timepoint/dt);
  if (absorptionAlpha.empty()) return 0.0;
  if(index > absorptionAlpha.size() - 1 ) return 0.0;
  if(index < 0) return 0.0;

  RF alphaIntergral = absorptionAlpha[index][cellNumber];
  return alphaIntergral;
}
