...
unsigned int N_ALPHA = 2;
unsigned int N_MATA = 1;
...
#include"../Bar1AlphaInstationary/bc/alpha_bc.hh"
#include"../Bar1AlphaInstationary/bc/bar1p_bc.hh"
#include"../Bar1AlphaInstationary/bc/diffusion_tensor_alpha.hh"
#include"../Bar1AlphaInstationary/bc/diffusion_tensor_bar1p.hh"
...
void driver(std::string const& meshfile, int level, int order, int dg_method, double dt, double tend) {
   ...
   switch(order){
     case 1:
       if(dim==2) driver_dg<GV,1,BLOCKSIZE2DP1>(gv, boundaryIndexToPhysicalEntity, dg_method, dt, tend, VTK);
       //if(dim==3) driver_dg<GV,1,BLOCKSIZE3DP1>(gv, boundaryIndexToPhysicalEntity, dg_method, dt, tend, VTK);
       break;
     case 2:
       if(dim==2) driver_dg<GV,2,BLOCKSIZE2DP2>(gv, boundaryIndexToPhysicalEntity, dg_method, dt, tend, VTK);
       //if(dim==3) driver_dg<GV,2,BLOCKSIZE3DP2>(gv, boundaryIndexToPhysicalEntity, dg_method, dt, tend, VTK);
       break;
     case 3:
       if(dim==2) driver_dg<GV,3,BLOCKSIZE2DP3>(gv, boundaryIndexToPhysicalEntity, dg_method, dt, tend, VTK);
