...
public:
                        ...
                        typename Traits::RangeType& y) const
  {
	...
	y = (double)physgroup_index;
	std::cout << "Physical group index: " << physgroup_index << "\t" << is->boundarySegmentIndex() << "element index: " << "\t" << gv.indexSet().index(e) << std::endl;
	return;
      }
    }*/
...
public:
  ...
  inline void evaluate (I& i, const typename Traits::DomainType& xlocal,
    ...
    /*if (GV::dimension == 3)
    {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC; ///(coordsOfCells[physgroup_index][2]); //Inflow
    }
    else {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC*Integration_height;
    }*/
    ...
    /*if (GV::dimension == 3)
    {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC/R; ///(coordsOfCells[physgroup_index][2]); //Inflow
    }
    else {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC;
    }*/
    
    if (GV::dimension == 3)
    {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC/(coordsOfCells[physgroup_index][2]); //Inflow
      if( (physgroup_index > N_ALPHA + 1) && (physgroup_index > 0) ) y = ALPHA_ENDO/(coordsOfCells[physgroup_index][2]); 
    }
    else {
      if( (physgroup_index < N_ALPHA + 1) && (physgroup_index > 0) ) y = - ALPHA_SEC;
      if( (physgroup_index > N_ALPHA ) && (physgroup_index > 0) ) y = ALPHA_ENDO;
    }
