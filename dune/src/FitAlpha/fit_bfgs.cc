...
static const double FMAX = 1507.0;
...
#include"../FitAlpha/bc/alpha_bc.hh"
#include"../FitAlpha/bc/diffusion_tensor_alpha.hh"
#include"../FitAlpha/bc/diffusion_tensor_bar1p.hh"
...
double rssq(const gsl_vector *p, void* params)
{
	...
	{
		F = FMAX * pow(absorptionAlpha.back()[i], N_H)/( pow(absorptionAlpha.back()[i], N_H) + pow(K_M, N_H) );		
		sum += gsl_pow_2(F - alpha[i]);
	}
...
int main(int argc, char** argv)
{
  try{
    ...
    if (argc != 3)
      {
        if(helper.rank()==0)
          std::cout << "usage: ./fit_bfgs <mesh file> <config file>" << std::endl;
        return 1;
      }
