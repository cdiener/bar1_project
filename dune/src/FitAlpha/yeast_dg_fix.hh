...
void yeast_dg (const GV& gv, const PGMap& pg, int dg_method,  bool vtk)
{
  ...
  if(vtk)
  {
    ...
    vtkwriter.write("solStat",Dune::VTKOptions::ascii);
    ...
    for(int i= (N_ALPHA +1); i < (N_ALPHA + N_MATA + 1 ); i ++ ) {
      ...
      std::cout << absorptionAlpha.back()[i - N_ALPHA -1] << std::endl; //correct for adsorption parameters
