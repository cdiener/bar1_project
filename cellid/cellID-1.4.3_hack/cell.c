...
int main(int argc, char *argv[]){
  ...
  // Wellcome message
  printf("\n*** Cell_ID Version 1.4.3 (mod) *** \n*** Give-me-more HACK by Christian Diener <diener@molgen.mpg.de> *** \n\n");
...
#define n_recomb_cuts_max 100
  ...
  for(i=1;i<argc;i++){
    if(strcmp(argv[i],"-?")==0||strcmp(argv[i],"--help")==0||argc==1){
      help_flag = 1;
      break;
    }
  ...
  for(i=0;i<n_fluor;i++){
    ...
    if ((treat_brightfield_as_fluorescence==0)||(flag_bf[i]==0)){
      ...
      strcat(line, "_BOUND");
      strcat(line, ".txt");
      printf("Writing cell boundaries to output file %s.\n",line);
      if(write_boundary_to_file(line)==0) printf("Couldn't output %s.\n", line);
      
      strcpy(line, fluor_files[i]);
      strcat(line, "_INT");
      strcat(line, ".txt");
      printf("Writing Interior points/fluorescence to output file %s.\n", line);
      if(write_interior_to_file(line)==0) printf("Couldn't open file %s.\n", line);
    }
